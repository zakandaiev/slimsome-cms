...
$user_add_query = $pdo->prepare("
  INSERT INTO ".$prefix."_users
    (login, password, email, nick, steam_id, name, service_id, service_start, service_end, service_nolimit, service_bind_type, isadmin, ismoder)
  VALUES 
    (:login, :password, :email, :nick, :steam_id, :name, :service_id, :service_start, :service_end, :service_nolimit, :service_bind_type, :isadmin, :ismoder)
");
...
if (empty($email)) {
  $email = null;
  $user_add_query->bindParam(":email", $email, PDO::PARAM_NULL);
} else {
  $user_add_query->bindParam(":email", $email);
}
if (empty($nick)) {
  $nick = null;
  $user_add_query->bindParam(":nick", $nick, PDO::PARAM_NULL);
} else {
  $user_add_query->bindParam(":nick", $nick);
}
if (empty($steam_id)) {
  $steam_id = null;
  $user_add_query->bindParam(":steam_id", $steam_id, PDO::PARAM_NULL);
} else {
  $user_add_query->bindParam(":steam_id", $steam_id);
}
if (empty($name)) {
  $name = null;
  $user_add_query->bindParam(":name", $name, PDO::PARAM_NULL);
} else {
  $user_add_query->bindParam(":name", $name);
}
if (!isset($_POST["service_id"]) || empty($_POST["service_id"])) {
  $service_id = null;
  $user_add_query->bindParam(":service_id", $service_id, PDO::PARAM_NULL);
} else {
  $user_add_query->bindParam(":service_id", $_POST["service_id"]);
}
if (!isset($_POST["service_start"]) || empty($_POST["service_start"])) {
  $service_start = null;
  $user_add_query->bindParam(":service_start", $service_start, PDO::PARAM_NULL);
} else {
  $user_add_query->bindParam(":service_start", $_POST["service_start"]);
}
if (!isset($_POST["service_end"]) || empty($_POST["service_end"])) {
  $service_end = null;
  $user_add_query->bindParam(":service_end", $service_end, PDO::PARAM_NULL);
} else {
  $user_add_query->bindParam(":service_end", $_POST["service_end"]);
}
$user_add_query->bindParam(":service_nolimit", $service_nolimit, PDO::PARAM_BOOL);
if (!isset($_POST["service_bind_type"]) || empty($_POST["service_bind_type"])) {
  $service_bind_type = null;
  $user_add_query->bindParam(":service_bind_type", $service_bind_type, PDO::PARAM_NULL);
} else {
  $user_add_query->bindParam(":service_bind_type", $_POST["service_bind_type"]);
}
$user_add_query->bindParam(":isadmin", $isadmin, PDO::PARAM_BOOL);
$user_add_query->bindParam(":ismoder", $ismoder, PDO::PARAM_BOOL);
