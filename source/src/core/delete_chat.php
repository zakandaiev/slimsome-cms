...
$chat_del_query = $pdo->prepare("DELETE FROM ".$prefix."_chat WHERE id=:chat_id");
$chat_del_query->bindParam(":chat_id", $chat_id);

// EXEC QUERY
try {
  $chat_del_query->execute();
  if ($chat_del_query->rowCount() == 1) {
    echo json_encode(array("success" => 1));
  } else {
    echo json_encode(array("error" => 0, "error" => "Неверные данные сообщения"));
  }
