...
if (!isset($_POST["news_id"])) {
  echo json_encode(array("success" => 0, "error" => "Неверный параметр новости"));
  exit();
}
...
$news_update_query = $pdo->prepare("
  ...
  WHERE id=:news_id
");
$news_update_query->bindParam(":news_id", $news_id);
if (empty($author)) {
  $author = null;
}
$news_update_query->bindParam(":author", $author);
$news_update_query->bindParam(":title", $title);
$news_update_query->bindParam(":url", $url);
if (empty($body)) {
  $body = null;
}
$news_update_query->bindParam(":body", $body);
if (empty($_POST["image_str"])) {
  $image = null;
} else {
  $image = trim($_POST["image_str"]);
}
$news_update_query->bindParam(":image", $image);
if (empty($meta_description)) {
  $meta_description = null;
}
$news_update_query->bindParam(":meta_description", $meta_description);
if (empty($meta_keywords)) {
  $meta_keywords = null;
}
$news_update_query->bindParam(":meta_keywords", $meta_keywords);
$news_update_query->bindParam(":enabled", $enabled, PDO::PARAM_BOOL);
$news_update_query->bindParam(":cdate", $cdate);

$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

try {
  $news_update_query->execute();
  if ($news_update_query->rowCount() >= 1) {
    generateSitemapXml();
